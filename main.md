...
```
# Уникнення префіксу `get` у назвах методів API  

При проєктуванні API не слід додавати префікс `get` до назв методів, оскільки це зайва інформація, яка і так зрозуміла з контексту. Це відповідає загальноприйнятій семантиці методів Apple, де доступ до властивостей або отримання даних реалізується без використання `get`.  
